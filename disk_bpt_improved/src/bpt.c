...
void db_print() {
    ...
    printf("Leaf: ");
    off_t fin = loc;
    page * leaf = load_page(fin);
    int increase = 0;
    int violation = 0;

    while (leaf->next_offset != 0){
        int i = 0;
        for (; i < leaf->num_of_keys; i++) {
            //char buffer[10];
            //sprintf(buffer,"%d", (int) leaf->records[i].key);
            if (increase < leaf->records[i].key) {
                increase = leaf->records[i].key;
            } else {
                violation = leaf->records[i].key;
            }
            printf("%d,", (int) leaf->records[i].key);
        }
        
        //strcat(result, " | ");
        printf("| ");
        off_t next = leaf->next_offset;
    ...
    int j = 0;
    for (; j < leaf->num_of_keys; j++) {
        //char buffer[10];
        //sprintf(buffer,"%d", (int) leaf->records[i].key);
        printf("%d,", (int) leaf->records[j].key);
    }
    ...
    if (violation == 0) {
        ...
        printf(" Validation : ERROR (key: %d) ", violation);
    }
    free(leaf);
    printf("\n");
}
...
off_t find_leaf_path(int64_t key) {
    int i = 0;
    page * p;
    off_t loc = hp->rpo;
    ...
    while (!p->is_leaf) {
        ...
        // return NULL;
        if (i == 0) {
            printf(" (next offset) %d -> ", p->b_f[0].key);
        } else {
            printf(" %d -> ", p->b_f[i - 1].key);
        }
...
off_t insert_into_leaf_redistribute(off_t leaf, off_t prev_offset, int k_prime_index, record inst) {
    ...
    insertion_index = 0;
    while (insertion_index < LEAF_MAX && leafp->records[insertion_index].key < inst.key) {
        insertion_index++;
    }
    for (k = 0; k < prev->num_of_keys; k++) {
        temp[k] = prev->records[k];
    }
    ...
    k = leafp->num_of_keys + prev->num_of_keys + 1;
    leafp->num_of_keys = 0;
    prev->num_of_keys = 0;
    split = cut(k);
    //printf("k: %d, split: %d, insertion_index: %d\n", k, split, insertion_index);
    ...
    for (i = split, j = 0; i < k; i++, j++) {
        leafp->records[j] = temp[i];
        leafp->num_of_keys++;
    }

    free(temp);

    for (i = prev->num_of_keys; i < LEAF_MAX; i++) {
        prev->records[i].key = 0;
        //strcpy(ol->records[i].value, NULL);
    }

    for (i = leafp->num_of_keys; i < LEAF_MAX; i++) {
    ...
    pwrite(fd, prev, sizeof(page), prev_offset);
    free(leafp);
    free(prev);
...
off_t insert_into_leaf_as(off_t leaf, record inst) {
    ...
    // 2022-11-23 이전 노드
    nl->prev_offset = leaf;
    page * nnl = load_page(nl->next_offset);
    nnl->prev_offset = new_leaf;
    pwrite(fd, nnl, sizeof(page), nl->next_offset);
    free(nnl);
...
off_t insert_into_internal_as(off_t bumo, int left_index, int64_t key, off_t newp) {
    ...
    // 2022-11-23 이전 노드 -> internal node의 경우 이전(왼쪽) 노드 가리키도록
    new_parent->prev_offset = bumo;
...
void coalesce_pages(off_t will_be_coal, int nbor_index, off_t nbor_off, off_t par_off, int64_t k_prime) {
    ...
    if (nbor_index == -2) {
        ...
        // 2022-11-23 이전 노드
        nbor->prev_offset = wbc->prev_offset;
    }
    else {
