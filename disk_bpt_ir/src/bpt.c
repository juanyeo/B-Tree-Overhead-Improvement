...
int db_insert(int64_t key, char * value) {
    ...
    page * leaf_prev = load_page(leafp->prev_offset);
    ...
    free(leaf_prev);
    if (prev_keys < LEAF_MAX) {
        insert_into_leaf_redistribute(leaf, leafp->prev_offset, nr);
        free(leafp);
        return 0;
    }
    /*
...
off_t insert_into_leaf_redistribute(off_t leaf, off_t prev_offset, record inst) {
    page * leafp = load_page(leaf);
